...
import {
  OrdersFilterPanel, EMPTY_FILTERS, countActiveFilters, type OrderFilters,
} from "@/components/ui/OrdersFilterPanel";
...
export default function OrdersPage() {
  ...
  const load = useCallback(() => {
    ...
    if (filters.releaseFrom)           params.releaseFrom      = filters.releaseFrom;
    if (filters.releaseTo)             params.releaseTo        = filters.releaseTo;
    if (filters.released)              params.released         = filters.released;
    if (filters.amountMin)             params.amountMin        = filters.amountMin;
    if (filters.amountMax)             params.amountMax        = filters.amountMax;
    if (filters.shippingMin)           params.shippingMin      = filters.shippingMin;
    if (filters.shippingMax)           params.shippingMax      = filters.shippingMax;
    ...
    if (filters.itemCount)             params.itemCount        = filters.itemCount;
    if (filters.isPack)                params.isPack           = filters.isPack;
    if (filters.shipmentStatus.length) params.shipmentStatus   = filters.shipmentStatus.join(",");
    if (filters.hasTracking)           params.hasTracking      = filters.hasTracking;
  ...
  }, [currentPage, search, onlyMissingCost, filters, sortField, sortDir]);
  ...
  const handleExport = async () => {
    setExporting(true);
    try {
      const { data } = await exportApi.orders(
  ...
  const panelFiltersCount   = countActiveFilters(filters);
  const activeFiltersCount  = panelFiltersCount + (onlyMissingCost ? 1 : 0) + (search ? 1 : 0);

  const clearAll = () => {
    setSearchInput(""); setSearch(""); setFilters(EMPTY_FILTERS);
    setOnlyMissingCost(false); setCurrentPage(1);
  };
        ...
        <div className="flex gap-2 flex-wrap">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input
              ...
              placeholder="Buscar por ID, produto, SKU, comprador ou CPF/CNPJ..."
          ...
          >
            ...
            Filtros
            {panelFiltersCount > 0 && (
              <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-brand/30">
                {panelFiltersCount}
              </span>
            )}
          </button>
